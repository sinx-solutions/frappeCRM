<template>
  ...
  />
  <BulkAIEmailModal
    v-if="showBulkAIEmailModal"
    v-model="showBulkAIEmailModal"
...
import BulkAIEmailModal from '@/components/Modals/BulkAIEmailModal.vue'
...
const showBulkAIEmailModal = ref(false)
  ...
  if (props.doctype === 'CRM Lead') {
    ...
    actions.push({
      ...
      onClick: () => {
        ...
        console.log("Raw selections:", selectionArray);
        
        if (selectionArray.length === 0) {
          console.log("Warning: No leads selected. Will use filters instead:", list?.params?.filters);
        }
        
        // Convert selections Set to an array of lead objects
        const selectedLeads = selectionArray.map(leadId => {
          console.log("Processing lead ID:", leadId);
          return { name: leadId };
        });
        
        console.log("Selected leads for AI emails:", selectedLeads);
        console.log("Current list filters:", list?.params?.filters);
        
        selectedValues.value = selectedLeads;
        showBulkAIEmailModal.value = true;
        capture('bulk_ai_email_modal_opened', { selection_count: selections.size });
      },
    })
  }
