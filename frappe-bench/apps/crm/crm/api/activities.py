...
@frappe.whitelist()
def get_activities(name, _force_refresh=None):
	"""Get activities for a lead or deal
	
	If _force_refresh is provided, we'll invalidate any docinfo cache to ensure fresh data
	"""
	# Log request for debugging
	frappe.log(f"ACTIVITIES: Getting activities for {name} with force_refresh={_force_refresh}")
	
	# If force refresh is requested, invalidate docinfo cache
	if _force_refresh:
		frappe.cache().delete_value(f"docinfo:{name}")
		frappe.log(f"ACTIVITIES: Cleared docinfo cache for {name}")
	
	if frappe.db.exists("CRM Deal", name):
		frappe.log(f"ACTIVITIES: Document is a CRM Deal")
		return get_deal_activities(name)
	elif frappe.db.exists("CRM Lead", name):
		frappe.log(f"ACTIVITIES: Document is a CRM Lead")
		return get_lead_activities(name)
	else:
		frappe.log(f"ACTIVITIES: Document not found: {name}")
